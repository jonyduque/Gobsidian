...
// Gravar substitui o conteudo do arquivo travado.
//
// Existe porque o conteudo default -- o PID que TentarTravar escreve -- responde
// a UMA pergunta ("quem detem?"), e internal/instalar precisa que o mesmo
// arquivo responda a outra ("quem e este processo, servindo qual cofre, em que
// versao?"). Escrever de fora exigiria um segundo descritor sobre um arquivo
// travado, ou um segundo arquivo ao lado -- duas contas do mesmo fato.
//
// O conteudo continua sendo SO diagnostico. Nenhuma decisao de exclusao depende
// dele, que e precisamente o que mudou em 2026-08-31: antes, um PID ilegivel
// autorizava outro processo a roubar o lock. Quem decide a posse e a trava.
//
// Escrever nao conflita com a trava: no Windows ela cobre a faixa em 1<<62 (ver
// faixaDaTrava), longe de qualquer conteudo real, escolhida assim para que
// outro processo consiga LER o arquivo enquanto este o detem.
func (t *TravaDeArquivo) Gravar(conteudo []byte) error {
	if t == nil || t.f == nil {
		return errors.New("trava nula")
	}
	...
	if _, err := t.f.Write(conteudo); err != nil {
		return err
	}
	return t.f.Sync()
}
