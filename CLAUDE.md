...
**Flag booleana ou inteira não distingue "omitida" de "definida com zero".** `config.Flags` tem companheiros `ReadOnlySet` e `DebounceMSSet`. **Toda** chamada a `config.Load` precisa preenchê-los com `cmd.Flags().Changed(nome)` — esquecer em um subcomando faz flag virar no-op silencioso. Vale também para `service.Options.SnippetCacheEntries`, que é `*int` pelo mesmo motivo: nil usa o padrão, zero explícito desliga o cache.

**Cache ligado no harness que mede latência transforma o requisito em outra pergunta.** Os laços de RNF-04 repetem a MESMA consulta 30 vezes, e `b.Loop` faz o mesmo — então um cache de trecho ligado acerta 29 das 30, e o p95 passa a descrever a consulta repetida, que nenhum usuário vê na primeira busca. Medido: `limit: 200` deu 21,49 ms quente contra 25,98 ms frio, e o quente teria entrado na documentação como "RNF-04 atingido" pelo motivo errado. Todo harness de latência desliga o cache por `semCacheDeTrecho`; quem mede repetição é um benchmark com "Repetido" no nome. **Otimização que só aparece quando a entrada se repete precisa de um benchmark que diga isso no nome.**

**Conserto que remove uma parada abrupta abre caminho que ninguém nunca executou.** O fix do pânico de `index.Build` com placeholder de nuvem tornou o boot possível — e, com ele, tornou alcançável `NotePaths() -> Inverted.Update -> os.ReadFile`, que baixa todo placeholder do cofre em segundo plano. Antes o processo morria antes de chegar lá. **Trocar um crash por violação de regra não negociável não é conserto completo, e é pior em uso**: o crash é visível e o download síncrono de milhares de arquivos trava a máquina do usuário sem dizer por quê. Depois de remover um `panic`, um `Fatal` ou um `return` de erro que abortava cedo, a pergunta é **o que passa a rodar agora que antes não rodava** — e a resposta se acha percorrendo os chamadores a jusante, não relendo a função consertada.

**Placeholder de nuvem chega ao índice sem `note`, e o coletor desreferenciava.** `build.go` manda `parsed{entry: e}` com `note` nil para anexo e para arquivo somente-nuvem, de propósito: ler dispararia download síncrono. `insert` entrava no ramo `r.entry.IsNote` — verdadeiro, é `.md` — e lia `r.note.Title`. Um único `.md` não hidratado do OneDrive derrubava `index.Build` no boot, com OneDrive sendo cenário suportado. Sobreviveu porque nenhum teste montava a condição: `FILE_ATTRIBUTE_RECALL_ON_DATA_ACCESS` não é gravável e o teste que tentava está pulado. **`FILE_ATTRIBUTE_OFFLINE` é gravável por `SetFileAttributes` e `vault.IsCloudOnly` também o aceita** — é assim que se monta o caso em teste. Continua aberto que `Build` registra o placeholder como `Note` e `index.Replace` o registra como `Asset`: mesma entrada, duas respostas conforme a construção.
