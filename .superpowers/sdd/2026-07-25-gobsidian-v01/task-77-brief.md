### Task 77: Fechamento do M6 e release v1.0.0

**Onde encaixa.** H5 do M6, e o fim da v1.0.

**A armadilha que esta tarefa existe para não repetir:** *não afirme estado que você não verificou.* O README já declarou **"v0.1 publicada"** sem tag, sem release e sem o gate de órfãos ter rodado. A frase custou nada de escrever e teria custado caro em quem confiasse nela.

#### Passos

1. Portão completo, com a saída de cada comando colada no relatório:
   - `pwsh -File scripts/verify.ps1` — as nove etapas.
   - `pwsh -File scripts/test_orphans.ps1 -Cycles 100`, incluindo os cenários novos da Task 76.
   - `pwsh -File scripts/audit_reports.ps1`.
   - `pwsh -File scripts/check_tool_params.ps1`.
   - `pwsh -File scripts/check_briefs.ps1`.
2. Binários para os três sistemas: `windows/amd64`, `linux/amd64`, `darwin/arm64`. Compilação cruzada, com o SHA-256 de cada um registrado.
3. Tag `v1.0.0` e release.
4. `docs/OPERACAO.md` com a tabela de RNFs completa — cada um com número medido ou **"não medido"**, e nada entre os dois.
5. Ledger em `.superpowers/sdd/2026-07-25-gobsidian-v01/progress.md` com as Tasks 69 a 77.

#### Verificações além dos passos

- **Confira todo SHA que você escrever no ledger.** A Task 31 foi registrada em `14210ee`, que não existe no repositório — `git cat-file -t` responde `fatal: Not a valid object name`. Ledger que aponta para o vazio é pior que ledger desatualizado, porque parece preciso. Rode `git cat-file -t` em cada um e cole a saída.
- Confirme que a tag existe **e** aponta para o commit certo: `git rev-parse v1.0.0` e `git log -1 --oneline`.
- Confirme que cada binário roda: `gobsidian version` em cada alvo disponível.
- **Não escreva "v1.0.0 publicada" antes de a release existir.** Verifique, depois escreva.

#### Prova de mutação

Esta tarefa **não tem prova de mutação** — o entregável é o portão e o release, não uma regra nova. O que a substitui é a saída real de cada comando, colada. "Testes passam" não é evidência; a saída do teste é.

#### Regras de execução

Idênticas às da Task 69, com ênfase no ledger: **registre antes de dizer que acabou.** Oito tarefas e onze commits já entraram neste projeto sem uma linha. A próxima sessão não tem seu contexto — ela tem o ledger.

#### Contrato de relatório

Grave em `.superpowers/sdd/task-77-report.md`: a saída de cada comando do portão, por inteiro; os SHA-256 dos três binários; a saída de `git cat-file -t` de cada SHA do ledger; a tabela final de RNFs; e o que ficou de fora, com o motivo.

Responda com no máximo 15 linhas.

**Files:** Modify `docs/OPERACAO.md`, `README.md`, ledger; create release artifacts
**Commit:** `docs: close M6 and release v1.0.0`
...
**Detalhado acima, como Tasks 63–68.** R1 virou a 64, R2 a 65, R3 a 66, R4 a 67; a 63 não estava aqui e entrou porque `note_move` precisa de offsets que o parser ainda não preenche para link Markdown, e a 68 é o fechamento.
