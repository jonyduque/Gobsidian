...
Task 146: complete (commit d5f784b, review clean — Sonnet; spec e qualidade
aprovadas sem achado; revisor conferiu as 22 linhas da tabela contra o plano).
Nota do revisor, fora do escopo: CLAUDE.md diz "14 etapas" e verify.ps1 tem
13 `Invoke-Step` — corrigir junto da próxima edição de CLAUDE.md.

Task 147 (impl Opus, commit 3dfcf8e, DONE_WITH_CONCERNS):
Ruling: o teste do watcher foi para `internal/watcher/varredura_raiz_ilegivel_test.go`
em vez do `varredura_raiz_test.go` do brief, porque esse arquivo já existia
(499e744) com dois testes sem build tag e o brief o chamava de "novo" —
custo se errado: um nome de arquivo a mais no pacote.
Ruling: a prova de mutação no ponto de chamada do watcher não executa em
Windows (`//go:build !windows`, `mutate.ps1` → EXIT=1 por "no tests to run");
vale a prova no ponto de chamada de `Walk` + `GOOS=linux go vet`, como o brief
previa — custo se errado: uma regressão no watcher só aparece em CI Linux.
Fora de escopo, anotado: instrumento de handle exclusivo duplicado no watcher
fecharia a prova em Windows (candidato a Task pequena depois da 181).
Task 147: complete (commits 3dfcf8e..a74b766, review Sonnet: A e B aprovados;
1 Minor — comentário do watcher.go:230 abria com a condição antiga — fechado em
a74b766, comentário só, conferido pelo orquestrador no diff).
Ruling: re-revisão dispensada para a74b766 — diff de 7 linhas de comentário,
lido pelo orquestrador — custo se errado: nenhum em código.

Task 148: complete (commit 1a22508, review Sonnet: A e B aprovados, sem achado;
revisor conferiu a fronteira mcpsrv — `ReadNoteItem` vai direto como `Out`,
nada a atualizar lá).

Task 149 — Ruling: a verificação 2 do brief cita "a tabela de erros de
`note_patch`" em TOOLS.md, que não existe (a seção tem schema e **Notas**, sem
tabela). Em vez de tabela nova, uma frase nas **Notas** de `note_patch`:
"`heading` e `block_id` juntos são `INVALID_ARGUMENT`." — e `docs/TOOLS.md`
entra no `git add` do commit — custo se errado: uma frase a mais numa seção
de doc.

Task 150 — Ruling: o `mutate.ps1` do brief acrescenta `"xyz"` à lista de
`ValidarEnum`, e o teste novo continua passando sob essa mutação (a mensagem
ainda lista os três reais e nenhum fantasma) — seria EXIT=1 e prova nenhuma.
A prova válida é a do Step 4 (tirar `"replace_section"` da lista): o teste
nomeia o modo ausente. O despacho manda rodar essa como a prova scriptada —
custo se errado: nenhum; a mutação do brief é a que não prova nada.

Task 149: complete (commit 4a06a24, review Sonnet: A e B aprovados, sem achado;
os 19 `CodeInternal` restantes em write.go são falhas de I/O do servidor).

Ruling (2026-09-03, owner: "Pode agrupar tasks pequenas"): batches of small, file-disjoint tasks go to ONE implementer dispatch and ONE review, keeping one commit per task as the plan mandates. Batches: [153+155] (daemon/lock.go vs daemon/daemon.go, disjoint); [154+156+157+158]; [160+163]; [162+164]; [166+167+168] (166/167 both touch mcpsrv/server.go, same agent edits sequentially); [178+179]. Alone: 151, 152 (write.go after 151), 159, 161 (overlaps 162/163 in bm25_test/persist_test), 165, 169, 170, 171-177, 180, 181. Batch model = highest tier of its members. — why: verify.ps1 dominates task wall-clock (~12 min/task measured on 148/149); a batch runs it once per dispatch. — cost if wrong: a fix round on one member re-reviews the batch diff; capped by keeping batches at <=4 tasks.
Note (2026-09-03): session sat idle ~18h with impl-150 parked on a background verify.ps1; resumed impl-150 by message. Edits were on disk, uncommitted.
Ruling (2026-09-03, owner: "utilize no maximo 3 agentes em paralelo"): cap honoured; effective concurrency stays 1 (implementer XOR reviewer) — why: all agents share one working tree; a reviewer's mutate.ps1 or verify.ps1 running while an implementer edits the same tree produces false reds or mutates a file mid-edit. Parallelism comes from batching, not from concurrent agents. — cost if wrong: wall-clock only.
Task 150: complete (commit 8cfec55, review clean — Sonnet). Note: brief's literal test slice tripped gocritic offBy1; implementer hoisted strings.Index with a guard, same assertion. Reviewer note: TOOLS.md:403 example JSON puts prose in "default" (not an enum value) — brief-mandated, park for the next TOOLS.md edit.
Ruling (2026-09-03): every dispatch carries "run the gate in the foreground; run_in_background/Monitor never wake a subagent here" — why: impl-150 and rev-150 both parked on dead background tasks (18h and ~7 min lost). — cost if wrong: none.
...
Note (2026-09-03, impl-151): TestRNF04SnippetConcurrencyLimit200 reds under machine contention (p95 47 ms vs 22 ms ceiling, 19.9 ms alone). Confirms the one-agent-at-a-time ruling; not a product defect.
Task 152: complete (commit a637b7f, review clean — Sonnet). Rulings: tests via createMoveService (package service_test); TOOLS.md edit on the note_move Retorno sentence; mutation anchor = one-line referrer error return -> `continue`.

Rulings for batch [154+156+157+158] (pre-checked against code 2026-09-03, before dispatch):
- Ruling: 154 is 53 lines (check_briefs CURTO) — accepted as-is; deletion task, no mutation proof — nothing to mutate.
- Ruling: 156 — no `escreve` helper exists in cmd/gobsidian tests; use `os.WriteFile`. `TestSubcommands_FlagsSetPopulated` and `TestIndexCmd_DebounceMSFlagZeroRejected` (cli_subcommands_test.go:168,:197) exercise the flags the task removes — delete both; debounce-zero rejection and flag-over-env are covered in internal/config/config_test.go:85-101 — why: a test of a flag that no longer exists cannot compile — cost if wrong: none, config tests own the rule.
- Ruling: 156 README table at README.md:194-208 only lists `serve` options; add a "Subcommands" column per brief, verifying each flag against `cmd/gobsidian/*.go`; `--max-results` is declared by serve, daemon, doctor, index, inspect, search today — after the task: serve, daemon, doctor, search (doctor/daemon out of scope, leave them). `--limit` (search, default 20) stays alongside `--max-results`.
- Ruling: 158 — brief says five `fmt.Errorf("index not available")` sites; code has FOUR (graph.go:87 LinkGraph, :303 TagList, :455 ListNotes, :556 NoteMetadata). VaultStats:663 has `if s.index == nil` with a vault.Walk fallback, not an error — leave it, drop the "Stats" case from the table test (or assert success). Method is `ListNotes`, not `NoteList`. Scripted mutation anchor is ambiguous across four identical lines → manual single-site mutation per brief fallback — why: brief written from prose, not from code — cost if wrong: one test row.

Task 153: complete (commit 5360a3b, report task-153-report.md; review pending — batched with 155). `[sem-doc]` hatch used; implementer justified in commit body and report.
Note (2026-09-03): impl-153-155 went idle three times mid-155 waiting on a backgrounded test_orphans run; I dispatched impl-155b (Sonnet) as a fresh finisher while the first agent was in fact still running the orphans gate — two writers on one tree, my error. Its first orphans run (daemon-idle 44/100 timeouts) is contaminated by its own concurrent rebuilds, not a regression. impl-155b hit the session limit; on reset, impl-153-155 told to stand down and impl-155b resumed as sole writer.
Ruling: an "idle" row in ListAgents does not mean the agent has no gate in flight — check Win32_Process for pwsh test_orphans/verify before dispatching a replacement — why: replacing an agent whose gate is still running puts two writers on one tree — cost if wrong: one wasted dispatch.
Ruling (2026-09-03, owner request): every dispatch requires a `## Progresso` section in the report file, one timestamped line appended at the end of each step before the next starts — why: agents that die mid-task left no trace of where they stopped; the orchestrator must be able to resume from the file — cost if wrong: a few lines of noise per report.
Ruling: implementers do NOT run scripts/test_orphans.ps1. Measured 2026-09-03: daemon-idle ≈ 13 s/cycle → ~22 min for that scenario alone, ~40 min for all four; the harness Bash cap is 10 min, so the call always times out and the agent goes idle mid-gate. The orchestrator runs the gate detached (Start-Process, stdout to a file) after the commit and before the review dispatch, and appends the result to the report — why: the gate exceeds any timeout an agent can set — cost if wrong: gate result lands in the report by a second hand.
Ruling: orchestrator polls liveness actively — ListAgents state + report mtime + gate processes — every ≤10 min; an idle agent without DONE gets one nudge, then a replacement.
Task 153: complete (commit 5360a3b, review clean — Sonnet, review-153-155.md).
Task 155: complete (commit cceb980, review clean — Sonnet; orphans gate 4x[OK] run detached by the orchestrator 23:51→00:30, appended to task-155-report.md). Non-blocking finding carried forward: docs/wiki/flows/encerramento.md:91 still lists three clean-disconnect errors; folded into the next batch as a doc-only commit. Known gap (reported, not hidden): daemon.go's `if !ipc.EhDesconexaoLimpa(err)` line is read-verified, not test-verified.
Ruling: progress-log timestamps must come from `Get-Date -Format HH:mm` / `date`, never typed — rev-153-155 wrote 00:10→00:40 for a run that took ~3 min — why: an invented timeline is an unmeasured number — cost if wrong: none.
Batch [154+156+157+158] implemented by impl-154-158 (Sonnet): 7f2ac8d (154), b1f2cfe (157), 3a1ab3e (158), b8ed7f6 (156), b354dcf (wiki follow-up from 155). Reports audited clean (audit_reports 154/156/157/158: no findings). Review dispatched to rev-154-158 (Sonnet), package review-cceb980..b354dcf.diff. Implementer went idle once mid-156 (background verify); one nudge sufficed. Out-of-scope notes from reports: daemon/doctor still declare --max-results/--read-only/--debounce-ms unaudited (156); write.go has several Errorf(CodeInternal) for I/O failures untouched (158).

Rulings for Task 159 (pre-checked 2026-09-04):
- Ruling: `boundedWait` differs today (daemon 5 s, ipc 3 s, serve 2 s); `vaulttest.Prazo = 5 s` replaces all three — a longer bound only lengthens a FAILING test — cost if wrong: slower red runs. `ponte_test.go` and `ipc/maxresults_handshake_test.go` also use boundedWait and must migrate too.
- Ruling: `search/cloudonly_update_windows_test.go:30` has a second `marcarSomenteNuvem` not listed in the brief — migrate it as well (six helpers become seven).
- Ruling: the dir-lock helper lives in `internal/vault/walk_raiz_windows_test.go:19` (package `vault`, internal), not walk_windows_test.go. An internal test of `vault` cannot import `vaulttest` (cycle). The file uses only exported identifiers (`New`, `Entry`, `v.Walk`) → convert it to `package vault_test` and migrate; if conversion proves impossible, keep the local helper with a comment pointing to vaulttest and say so in the report.
- Ruling: docs/papeis/testador.md has no "handle exclusivo" section today — add a short one (where Windows-specific tests are discussed) pointing to `internal/vaulttest`. docs/ESTRUTURA.md:229-230 shows a stale graph (mcpsrv → service → …) — out of scope for 159; noted for the final message.

Batch [154+156+157+158] review (rev-154-158, Sonnet, review-154-158.md): 154/157/158/b354dcf approved on spec and quality; 156 NOT approved on spec — README.md flag table listed only `serve` for six flags the code also declares in `daemon`/`doctor`. Fix round 1 (impl-154-158 resumed): 8bb4769 (README rows now match `grep -n 'Flags()' cmd/gobsidian/*.go` row by row, verified by the orchestrator too), 574eb21 (wiki formato-do-cache.md: search cache derives magic/codec from CacheFormatVersion; source_commit b1f2cfe, updated_at 2026-09-04), 86f07e5 (orchestrator: the wiki sentence dated the change 2026-09-02, b1f2cfe is 2026-09-04). Scoped re-review rerev-156 (Haiku, rereview-156-fix1.md): APPROVED.
Task 154: complete (commit 7f2ac8d).
Task 156: complete (commits b8ed7f6, 8bb4769). Out of scope, carried: daemon/doctor declare --read-only/--debounce-ms/--max-results (and daemon --log-level/--cache-dir/--eager-search) — whether they honour them is unaudited.
Task 157: complete (commits b1f2cfe, 574eb21, 86f07e5).
Task 158: complete (commit 3a1ab3e). Out of scope, carried: write.go still has Errorf(CodeInternal) at I/O-failure sites.
Task 159 implemented by impl-159 (Opus): c81b2b8 (code+docs) + 5377559 (report SHA). Report audited (audit_reports 159: only SECAO-AUSENTE TDD/RED-GREEN — the report carries "Prova 1/2" mutation sections with pasted FAIL output instead; accepted). Deviations D1–D6 declared in the report; D4 (dir-lock helper Fatalf where vault's helper Skipped) and D5 (ipc socket-absent ceiling 3 s → 5 s) flagged to the reviewer. Orphans gate launched detached 02:23 (PID 44884, orphans-159.log). Review dispatched to rev-159 (Opus), package review-86f07e5..5377559.diff. Note: the implementer committed task-159-report.md — .superpowers/ is tracked in this repo (376 files), so not a defect; prior tasks in this plan left reports uncommitted.
Task 159 review (rev-159, Opus, review-159.md): Spec APROVADO, Quality APROVADO, 0 blocking, 9 non-blocking. Adjudication: (1) ipc_test.go:64/:235 use Prazo as a latency ceiling, not a wait — FIX with a local ceiling; (2) D5 under-reported — covered by (1); (3) "cinco cópias / só uma conferia" in testador.md:142 and vaulttest/doc.go:8 — reviewer measured 8 sites / 4 proving — FIX both texts; (4) abrirExclusivo Skips where two old copies Fatal'd — Ruling: keep the Skip, it names its reason; two tests trade a named failure for a named skip only when CreateFile itself fails — cost if wrong: an environment that cannot lock goes unnoticed as a skip; (5) build_descarte_unix_test.go chmod 0000 proves nothing as root — FIX with the symmetric ReadFile proof; (6) move_atomico guard message blames the fixture when err==nil — FIX by adding err to the message; (7) CLAUDE.md go list claim holds only on GOOS=windows — FIX wording; (8) CLAUDE.md tree lacks vaulttest — FIX one line; (9) "14 etapas" — Task 163 adds the SKIP-count step, which closes it. Fix round 1 for 159 is DEFERRED until impl-160-163 commits: two writers on one tree already cost a contaminated gate once, and 163's measurements must not overlap a verify.ps1 run — why: sequencing beats speed here — cost if wrong: ~1 h later close-out of 159, nothing downstream depends on these fixes.
Batch [160+163] dispatched to impl-160-163 (Opus) at 02:27, base 5377559. Rulings: 160 Step 1 mutation targets internal/vault/walk.go:166 (overflow.go has no ctx check); 160 Step 4 sites are :169/:202/:235 (`data`, not `got`); :101/:128 left alone (they already assert on len); 163 corpus at %TEMP%\vault_5000 kept if 5000 notes, else regenerated Seed 42; ceilings from PRD.md:288-294 with the existing raceEnabled const; the SKIP-count step derives from the existing go test output (no second pass) and makes verify.ps1 14 steps, CLAUDE.md sentence updated; measurements only after the detached orphans gate (PID 44884) exits.
Orphans gate for 159 (detached, 02:23→02:44): stdin-eof [OK] 100/100, parent-death [OK] 100/100; signal and daemon-idle REFUSED by the gate's staleness check ("bin\gobsidian.exe e mais antigo que schema_params_test.go") because impl-160-163 was editing that file mid-run. Ruling: accepted for 159 — c81b2b8 changes no file compiled into gobsidian.exe (vaulttest is imported only by _test.go; go list confirms no production importer), so the gate was insurance, not evidence; the full four-scenario gate re-runs after the last task that touches production code and before the final review — why: a gate that cannot see the change proves nothing about it either way — cost if wrong: an orphan regression from 160–164 (all test-only) would surface only at the end-of-plan run.
Task 160: implemented by impl-160-163 (Opus), commit ef2bbb4 at 02:44; review pending, batched with 163.
Task 163: implemented by impl-160-163, commit d622afd at 03:18 (verify.ps1 14/14, "[!] 6 testes pulados"). Parity test now excludes LinkExternal on the got side and unions tags+frontmatterTags on the want side — flagged to the reviewer as the judgment call of this task. Review of [160+163] dispatched to rev-160-163 (Opus), package review-5377559..d622afd.diff. Fix round 1 for 159 dispatched to impl-159 (resumed) at 03:20 — the only writer on the tree. Note for future dispatches: a Win32_Process check written as `CommandLine -match 'test_orphans'` matches its own shell chain (4 PIDs with no gate running); filter on `Name -eq 'pwsh.exe'` and `-File.*test_orphans`.

- 2026-09-05 22:49 — recovery after the session-limit deaths (~03:3x, "resets 3:50am"). Tree check: `go build ./...` OK, vet clean; impl-159's fix round 1 already on the tree for all six findings (1,3,5,6,7,8), uncommitted, report Progresso incomplete. Resumed `impl-159` (R=1 continues; date nit 09-04→09-05, verify, explicit-path commit) and `rev-160-163` (restart from zero, diff-file only, Progresso at top of review file). One writer, one reader.
- 2026-09-05 23:05 — Task 159 fix round 1 landed: `4c08910` (six findings 1,3,5,6,7,8) + `facfe32` (report SHA). First attempt `a8156d1` had a stray `@ ` in the subject (heredoc artifact); the implementer caught it and amended before I did. Scoped re-review pending.
- 2026-09-05 23:05 — `review-160-163.md` written (470 lines). Task 160: Spec APPROVED, Quality APPROVED. Task 163: Spec APPROVED, Quality NOT APPROVED — B1 `rnf5000_test.go:29` "~100 ms" vs measured median 371.5 ms; B2 `:222` asserts `-Seed 42` that the report calls unverified. Nine non-blocking N1–N9.
Task 160: complete (ef2bbb4)
- Ruling: N3 (`getVault5000Path` still Skips when `%TEMP%\vault_5000` is absent) stays as is — the vault is environment, and verify.ps1 step 3 now counts the skip out loud; the report gets one line saying so — cost if wrong: a machine without the vault stays green-without-measurement, but visibly.
- Ruling: N8 (Logf of `rebuildDur/loadDur` ratio) not applied — Task 163's own point was budgets replacing Logf; the assertion is the guard — cost if wrong: a 5% inversion is indistinguishable from a 6x one until it flips.
- 2026-09-05 23:06 — fix round 1 for Task 163 dispatched to `impl-160-163` (R=1): B1, B2, N1, N2, N4, N5, N6, N7, N9 + the N3 report line.
- 2026-09-05 23:15 — `rereview-159-fix1.md`: Verdict APPROVED, six of six addressed; one precision note (CLAUDE.md:70 unaccented) fixed by the orchestrator in the commit below.
Task 159: complete (c81b2b8, 5377559, 4c08910, facfe32, b1fc99d)
- 2026-09-05 23:25 — Task 163 fix round 1 landed: `15c66fb` (B1, B2, N1, N2, N4, N5, N6, N7, N9 + N3 report line; verify 14/14 green, 6 skipped). Scoped re-review dispatched to `rev-160-163` (diff-file only).
- 2026-09-05 23:26 — Task 161 dispatched to `impl-161` (Opus), base `15c66fb`. Pre-check: `limites_enums_test.go:44`→`:45`; `cli_subcommands_test.go:168` (`TestSubcommands_FlagsSetPopulated`) already deleted in b8ed7f6 — row stays with that verdict; other eleven sites match. Writer alone; re-reviewer reads only the diff file.
- 2026-09-05 23:30 — `rereview-163-fix1.md`: NOT APPROVED. O1 BLOCKING `rnf5000_test.go:223` — the B2 rewrite wrote `\v` inside the comment (VT byte; path reads `%TEMP%<VT>ault_5000`). O2 non-blocking `parity_test.go:29-30` — `slices.Compact` inside `conjuntosIguais` drops multiplicity for all five categories, not only tags (two identical headings or two links to one target now collapse; before the fix round that reproved). Eight of ten ADDRESSED. Extra commits `9629584`, `50fb0bc` touch only task-163-report.md.
- Ruling: O2 gets fixed in round 2 (Compact only on the tags comparison, the alternative the original N4 offered) — a comparison that stops counting is a weaker guard than the one the task replaced — cost if wrong: none beyond one more edit.
- Ruling: Task 163 fix round 2 is QUEUED until `impl-161` reports — 161 does hand mutations across service/search/index and a concurrent verify.ps1 from either writer would fail spuriously; one writer on the tree.
- 2026-09-05 23:59 — Task 161 implemented: `b3bcba0` (8 `_test.go` files, verify 14/14) + `ebc0f12` (report SHA). Fourteen-row table (write_test triple split). Nudged once: one Progresso timestamp was typed, not read. Review dispatched to `rev-161` (Opus) with `review-15c66fb..ebc0f12.diff`. Task 163 fix round 2 (O1, O2) resumed on `impl-160-163` — writer alone.
- 2026-09-06 00:06 — `review-161.md`: Spec APPROVED, Quality APPROVED, 0 blocking, 5 non-blocking (TestPoolReuse name promises reuse; only vault_search had a pasted mutation of six tools; config_test.go:203-208 silent return — out of scope; bm25_test.go:251 ordering passes by 3% under a tf/length confounder; limites_enums writes 501 files per run — cost accepted). Implementer also committed `4ffaf50` (typed-time line corrected).
Task 161: complete (b3bcba0, ebc0f12, 4ffaf50)
- Ruling: 161 polish round (queued behind 163 round 2, one writer): rename `TestPoolReuse`; one sentence in the report declaring the proof covered vault_search only (the brief's unit is the site, and the site got its proof); bm25:251 asserts only what the idf rule requires — cost if wrong: one more small diff. config_test.go silent return parked for the out-of-scope list; the 501-file cost noted.
- 2026-09-06 00:15 — Task 163 fix round 2 `b19316d` + `44f2700`: re-review Round 2 APPROVED, 0 open. 
Task 163: complete (d622afd, 15c66fb, 9629584, 50fb0bc, b19316d, 44f2700) — out-of-scope carried: corpus 1.4 MB vs PRD 50 MB; RNF-02 has no ceiling; seed unverified (now said so in the comment); parity universe corrections; N3 Skip deliberate; config_test.go:203-208 silent return (from review-161).
- 2026-09-06 00:15 — Task 161 polish round `6c1f217` (rename `TestNormalizeAcentosECaixa`, bm25 ordering line removed with the derived-not-measured 1.507/1.457 note, report sentence on proof scope). Ruling: re-reviewed by the orchestrator — a two-file diff of a rename, a comment and one removed assertion, idf mutation FAIL pasted, verify green in the report — cost if wrong: a missed nit in a comment. Task 161 commits now b3bcba0, ebc0f12, 4ffaf50, 6c1f217.
- 2026-09-06 00:20 — Batch [162+164] dispatched to `impl-162-164` (Opus), base `6c1f217`, two commits expected. Pre-check: bm25_test.go:127→:133; `atualizaGolden` flag exists at ranking_golden_test.go:22; go-cmp is indirect only (no promotion); watcher has `esperaContador`/`esperaTermo`; delete_test mtime site now :213 plus a Sleep(10ms) at :125 added to the protocol.
- 2026-09-06 00:42 — Task 162 implemented: `d12f84f` (bm25_test, ranking_golden_test, 6 regenerated tsv; 12 files). Review dispatched to `rev-162` (Opus) with `review-a313f32..d12f84f.diff`. Implementer continues on 164.
- 2026-09-06 00:55 — `review-162.md`: Spec APPROVED, Quality NOT APPROVED. B1 `ranking_golden_test.go:88,:218-219` + commit message claim the old golden was byte-inert to the heading weight; reviewer reproduced the old corpus via `go test -overlay` and `dois-termos.tsv`/`so-em-heading.tsv` changed scores (order identical). N1 `:139-142` "demais notas trazem a forma reduzida" false (only i%3==0). N2 `so-em-heading` order is decided by length; heading weight is a common factor → say so. N3 `n0000` is the unique shortest note and in every congruence class; wins 3 of 6 goldens.
- Ruling: N3 applied — filler becomes `20 + ((i*7+29)%60) + i/60` (the reviewer's phase shift), six tsv regenerated. Deviates from the brief's verbatim formula because that formula recreates the "shortest note wins" confounder the task exists to remove — cost if wrong: one testdata regeneration.
- Ruling: B1's commit message cannot be rewritten (no amend of a landed commit); a correcting paragraph in the report plus the two docstrings in the fix commit is the remedy.
- Fix round 1 for 162 queued until the 164 commit lands (same writer).

- 2026-09-06 01:02 Task 164 landed: af93306 (watcher sleeps -> observable signals; espera_test.go; delete_test mtime). Review package review-d12f84f..af93306.diff; rev-164 (Opus, read-only) dispatched. Task 162 fix round 1 (B1/N1/N2/N3) dispatched to impl-162-164 as sole writer.
- 2026-09-06 01:12 review-164.md: Spec NOT APPROVED (report-only: B1 report still says EM ANDAMENTO, no SHA/table/-count=20 output/gate line; B2 23/200 without its command), Quality APPROVED. N1 CLAUDE.md vaulttest importer list lacks watcher; N2 commit says "thirteen" sleeps, diff counts 12; N3 setupTestWatcher t.Cleanup(cancel) before EsperarWatcherAtivo; N4 TestCounters_Reconciliations strict subset of TestRun_OverflowSchedulesExactlyOne; N5 TestDebounce_Coalescence changed shape (stronger), unreported.
  Ruling: N2 corrected in the report, not by amending af93306 — same as 162/d12f84f; history stays linear — cost: one wrong count in git log.
  Ruling: N4 apply, delete TestCounters_Reconciliations — same rule Task 161 used for redundant tests; overflow_test asserts the exact value and the non-overflow control — cost: a counter test lives one file away from its name.
  Ruling: N1, N3, N5 apply. Fix round 1 for 164 queued behind the 162 fix commit (single writer).
- 2026-09-06 01:17 Task 162: complete (d12f84f, 4c6da0a — fix round 1 amended c572e83 in place; review-162.md Round 1: B1/N1/N2/N3 ADDRESSED, Spec APPROVED, Quality APPROVED; heading-weight mutation re-run by the reviewer via overlay: so-em-heading FAIL).
- 2026-09-06 01:25 Task 164 fix round 1 landed: 8568b5b. review-fix1-af93306..8568b5b.diff packaged; rev-164 scoped re-review dispatched. Task 165 dispatched (Opus, impl-165), base 8568b5b.
  Ruling (165 brief drift, pre-checked): Step 1 site is tools_read_test.go:349-373 and the limit is the constant maxPathsPorLote — test uses the constant, mutation anchor "len(req.Paths) > maxPathsPorLote"; Step 3 doctor/daemon.go does NOT read the 1<<62 offset — it calls daemon.TravaEmUso (:206) and os.ReadFile's the PID (:216) — the offset test keeps its purpose (byte 0 must stay readable while the lock is held) and adds the doctor-read assertion; the helper-process pattern already exists in trava_test.go (lancarAjudante, varAjudante, TestAjudanteSeguraTrava, package daemon internal) — reuse it, do not build a second one; Step 4 AliasCollisions() is a method on *Index (query.go:158) counting aliases with >1 paths; Step 5's "force ReadOnly=false in encode" mutation is already killed by ipc_test.go:184-205 — the live mutation is `want := HandshakeConfig{ReadOnly: readOnly` -> `ReadOnly: false` in DialAndHandshake; Step 6 test_orphans.ps1 is run by the orchestrator, not the implementer — cost if wrong: an extra fix round.
- 2026-09-06 01:32 review-164.md Round 1: seven items ADDRESSED; Spec APPROVED; Quality NOT APPROVED on NB1 (report claimed the complete version was on disk "desde 00:59" — false, the reviewer proved it with size/mtime/git log), NB2 Status lacked 8568b5b, NB3 CLAUDE.md grep returns a ninth line (internal/vaulttest's own external test), NB4 line over column 80.
  Ruling: orchestrator fixed NB1-NB4 directly in a7ee204 (text only, no code; check_doc_refs [OK]) instead of a second fix round — impl-165 is the active writer and a report/CLAUDE.md edit does not touch its files — cost: no reviewer eyes on 17 changed lines of prose.
- 2026-09-06 01:32 Task 164: complete (af93306, 8568b5b, a7ee204).
- 2026-09-06 01:47 Task 165 landed: 524e070 (9 _test.go files incl. internal/mcpsrv/export_test.go exporting MaxPathsPorLote — Task 167 must EXTEND that file). Report declares 7 brief deviations and a latent race at ponte.go:193-202 (ctx.Done vs hostParaDaemon on stdin EOF; 100/100 green under -race, not fixed, out of scope). review-a7ee204..524e070.diff packaged; rev-165 (Opus, read-only) dispatched. Batch [166+167+168] waits for rev-165 to finish its go test runs.
- 2026-09-06 01:59 review-165.md: Spec APPROVED, Quality APPROVED, 0 blocking. Reviewer re-ran all six mutation proofs (exit 0, same failing line numbers), vet windows/linux/darwin clean, 1200 extra -race rounds could not flip the ponte race. Reviewer typed four Progresso timestamps (up to 23 min ahead of the clock); nudged once; it rewrote them with a dated correction note. NB1 ponte_test.go:330 needs a comment naming the product race at ponte.go:193-202 (ctx.Done vs hostParaDaemon) so a CI red is not read as an M8 regression; NB2 ipc_test.go:249-258 accept goroutine swallows Accept/Greet errors; NB3 trava_kernel tests inherit lancarAjudante's t.Skipf (pre-existing).
  Ruling: NB1(b) and NB2 ride on the next writer dispatch (batch 166-168) as a separate commit; NB1(a) — the product fix (drain hostParaDaemon non-blockingly inside the ctx.Done branch) — is OUT OF SCOPE for this plan, goes to the final out-of-scope list for the owner; NB3 keep as is (pre-existing, verify.ps1 step 3 exposes the skip count) — cost if wrong: one comment and one channel of prose land under a batch commit instead of their own review.
- 2026-09-06 01:59 Task 165: complete (524e070, a4a5464 report SHA line).
- 2026-09-06 01:59 Batch [166+167+168] dispatched (Sonnet, impl-166-168), base a4a5464. Rider: Task 167 EXTENDS internal/mcpsrv/export_test.go (created in 524e070).
- 2026-09-06 02:01 Ruling (Task 169 normalizeKey, pre-checked): index/chave.go does NOT delegate to text (chaveDeCaminho = ToLower(text.ParaNFC(ToSlash)) inline) and vault.CanonicalPath is not NFC-normalized (no ParaNFC under internal/vault), so writer's ToLower-only lock key can give NFC and NFD spellings of one file two locks. Decision: add text.ChaveDeCaminho(string) string holding that formula; index.chaveDeCaminho delegates to it; writer.normalizeKey calls it; new production edge writer -> text, justified by "uma conta por regra", CLAUDE.md graph block updated in the same commit (writer -> parser, text, vault). chaveDeNomeDeArquivo and aliasKey stay in index (different rules). Cost if wrong: one extra edge from writer to a leaf; reversible in one commit.
- Ruling: Tasks 166 and 167 may land in ONE commit (message lists both scopes; separate reports; 168 separate) - the implementer had already mixed both in the tree and hand-splitting hunks across server.go/persist.go risks more than it buys - cost if wrong: coarser history, a revert of 166 drags 167 with it.
- 2026-09-06 03:01 Task 166 implemented: e46654d (report task-166-report.md; brief step 3 MoveNote Resolve swap BLOCKED by the implementer, kept with a regression test). Task 167 implemented: ca4cd0d. Both await batch review with 168.
- 2026-09-06 03:58 Task 166: complete (e46654d + fix 230250c; review-166-168.md Round 1 APPROVED/APPROVED; brief step 3 MoveNote Resolve swap ruled BLOCKED-justified: filepath.IsLocal("COM1")=false but IsLocal("COM1.md")=true, measured by reviewer). Task 167: complete (ca4cd0d). Task 168: complete (6b45897; depth default deleted from mcpsrv because service graph.go:90-96 already applies it, measured). Rider 8f801f3 approved.
- 2026-09-06 04:05 Task 169 dispatched to impl-169 (Opus), base 2307cf3; normalizeKey ruling carried in the dispatch.
- 2026-09-06 05:30 Task 169 implemented: 8268507..383fd90 (11 commits; item 2 []TagNode reverted in 383fd90 because the MCP SDK schema inference rejects the recursive type; item 3 narrower than brief - ix.tags as source would change the tool answer). DONE_WITH_CONCERNS; review dispatched.
- 2026-09-06 06:00 Task 169: complete (8268507..383fd90 + a6521d1 for review N1/N5; review-169.md APPROVED/APPROVED). Notes: commits 06c8640..d7a11e8 do not boot (mcpsrv.New panics on the recursive []TagNode schema) - git bisect over that interval will hit them (N2). Parked for the owner: graph.go edge sort not stable on Alias/Anchor ties, pre-existing (N3); resolve.go alias branch now accepts assets via vivosLocked, unreachable today, no test guards the invariant (N4); item 8 lock key cost per Lock unmeasured (implementer concern 1); a mcpsrv test that registers all tools and fails with its own message would have caught item 2 earlier (concern 4).
- 2026-09-06 06:00 Ruling: item 3 of Task 169 stays narrower than the brief (ix.tags as the flat source changes the tool answer - reviewer confirmed with data) - cost if wrong: TagListHierarquico keeps a per-note walk that the brief hoped to drop.
- 2026-09-06 06:03 Task 170 dispatched to impl-170 (Opus), base a6521d1; ruling: per-candidate base binaries built from the tree before the edit (antes_* binaries predate Task 169's service/index changes).
- 2026-09-06 06:33 Task 170 implemented: 8e0670c (removeLocked reverse map, -70.64% sec/op p=0.001, B/op +8.33%), 6ec12da (RewriteLinks one pass, -96.00% sec/op). Candidates 2, 4, 5 not entered (no sec/op gain). Ruling: candidate 4 stays out despite B/op -0.27% p=0.001 - Task 82 precedent, two exported methods with a silent precondition for a memory delta below noise - cost if wrong: 0.3% allocs on the cached broad-term path. Review dispatched to rev-170.
- 2026-09-06 06:55 Task 170: complete (8e0670c, 6ec12da, reports 314d930/dcae200, fix 5a1c320; review-170.md Round 1 APPROVED/APPROVED). Brief defect recorded: TestIndiceRecarregadoEIdenticoAoConstruido never calls Remove and cannot guard removeLocked; brief baseline 6.549 s for InvertedUpdateLote does not reproduce (3.936 s measured); candidates 2 and 5 pointed at code their paired benchmark never executes.
- 2026-09-06 07:00 Task 171 dispatched to impl-171 (Opus), base 8a70044. Brief correction carried: caller grep is 6 calls + 2 comment lines today (brief said 7); vault.Walk is a method on *Vault taking func(Entry) error.
Task 171 BLOCKED at step 6: verify.ps1 12/14, golangci-lint SA1019 on the six callers of the writer forwarders (staticcheck is on). Ruling: forwarders keep the forwarding prose but drop the `// Deprecated:` marker ("Encaminhador transitorio para vault.X; a Task 172 migra os chamadores e o remove.") — PR1 must not touch callers, `//nolint` in six sites and a lint exclusion both outlive the one-task window, merging 171+172 defeats the move/migrate split — cost if wrong: one task without tooling strike-through on five call sites, removed by Task 172 anyway.
Task 171: complete (4d7f397). Review review-171.md: spec OK, quality OK, 0 blocking. Ruling: N2 (cache temp-name literals in index/persist.go:114 and search/persist.go:69), N4 (forwarder surface is 8 lines incl. servico.go:73 SweepResult) and N5 (ReplaceFile docstring must state the callback contract: do not close/rename the file, flush buffers) are carried into the Task 172 dispatch — 172 already rewrites both persist files and writes the two callbacks — cost if wrong: 172 grows by one docstring sentence. N1 (ARCHITECTURE.md §5.5 says 3 retries/50 ms, code does 10/10 ms, and omits dir fsync + Chmod) and N3 (ESTRUTURA.md lists vault/ignore.go and writer/writer.go, neither exists) are pre-existing doc drift: parked for the final doc pass after Task 181, not folded into a code task.
Task 172 dispatched to impl-172 (Sonnet), base 011042f. Corrections carried: 8 forwarder lines (5 WriteAtomic calls in write.go, servico.go:73/:78/:103), writer block_test/section_test call writer.WriteAtomic and move to vault.WriteAtomic, 171 N2/N4/N5 folded in, per-task base binaries antes172_*, orphans gate stays with the orchestrator.
Task 172 implemented (ae619cf); review-172.md: spec approved with reservation, quality good, 1 blocking (N1 docs still describe the deleted writer/atomic.go). Fix round 1 -> impl-172: N1, N2, N3, N5 as written. Ruling on N4 (cache files now born 0644 instead of 0600 via ReplaceFile): accepted, documented in ESTADO.md's cache-format section, no signature change — the cache dir has been created 0755 under os.UserCacheDir all along, so the file mode follows the directory's posture; notes are 0644 for the same reason — cost if wrong: on a multi-user Unix host another local user can read cache contents (paths, tokens), which a later task can close by giving ReplaceFile a mode parameter. Process deviation noted: impl-172 ran verify.ps1 in background despite the foreground rule; result accepted (last line pasted, exit 0).
Task 172: complete (ae619cf, fix 68c0bc5). Round 1 re-review: N1-N5 addressed. N6 (segregacao.md section 2 title and opening citation still describe three implementations) parked: the document is historical and its closing line records the extraction — cost if wrong: a reader of a dated audit skims a moot recommendation. Orchestrator ran verify.ps1 -SkipCross -SkipNet on 68c0bc5 independently (see next line).
Orchestrator verify.ps1 -SkipCross -SkipNet on 68c0bc5: [OK] Bateria completa. Pode commitar. (08:26-08:29). Task 173 dispatched to impl-173 (Sonnet), base c28bc86. Corrections carried: search.go assertion is at :226-228 with uses at :232 and :310 (brief said :202-204/:208/:298); ESTADO.md mention is at :157 (brief said :136); per-task base binary antes173_service from the pre-edit tree.
Task 173 BLOCKED at step 3: internal/service/frontmatter_err_test.go:55 asserts svc.index.(*index.Index) to reach Replace. Ruling: not a fake, a convenience assertion the change makes invalid — replace with `ix := svc.index`, drop the now-unused import, add the test file to the commit paths; the brief's "no test edited" rule guards against fakes, and the pre-check found none — cost if wrong: none observable, the test keeps the same assertions.
Task 173: complete (c1ef235). review-173.md: spec PASS, quality PASS, 0 blocking; N1 (benchstat table described, not pasted) fixed by the orchestrator: table appended to task-173-report.md from the raw Step 5 files, all ~.
Task 174 dispatched to impl-174 (Opus), base 7119645. Extract internal/boot (AbrirIndice, PrepararBusca, Componentes, Montar) from cmd/gobsidian servico.go/serve.go. Orphans gate carried by orchestrator after the commit.
Task 174 impl commit eac13c9 (DONE_WITH_CONCERNS, 8 concerns; verify 14/14 after an OPERACAO.md:1525 check_doc_refs dispensation). Ruling: concerns 1-3 accepted (verbatim bodies beat the brief's illustrative code; no forward reference to Task 177 in doc.go; comment names serveEmProcesso) - move-not-rewrite is the binding rule - cost if wrong: two log strings differ from the brief's wording. Concerns 5 (stale .go comments citing serve.go) and 6 (docs/wiki) parked for the final doc pass. Review package review-7119645..eac13c9.diff; rev-174 (Opus) dispatched read-only. Orphans gate launched detached 09:35 (pwsh PID 3840, log orphans-174.log). Bench daemon PID 51516 on vault_5000 left from impl-174's measure - not mine, left to idle out.
Task 174 review: approved with should-fix N1-N5 (all docs), nits N6-N8 parked. Fix round 1 by impl-174 -> 33cff51 (4 files, 11 lines). Ruling: orchestrator re-reviewed the 11-line doc diff itself against N1-N5 instead of dispatching a re-reviewer - each finding is a named sentence and the diff shows each replaced - cost if wrong: a stale doc sentence survives to the final doc pass. Orphans gate run 1 (09:35-09:48, log orphans-174.log): stdin-eof 100/100 OK, parent-death 100/100 OK; signal and daemon-idle REFUSED to run because impl-174's fix edited doc.go after bin/gobsidian.exe was built (freshness check). Rebuilt at 33cff51, signal + daemon-idle rerun detached 09:49 (pwsh PID 26048, log orphans-174b.log). Lesson: no edits to the tree while the gate runs, not even comments.
Orphans gate rerun (09:49-10:13, orphans-174b.log): signal 100/100 (reason signal), daemon-idle 100/100 (reason idle). With run 1: 4/4 scenarios green on the Task 174 tree. verify.ps1 -SkipCross -SkipNet on 33cff51: [OK] Bateria completa (10:14). Task 174: complete (eac13c9, 33cff51).
Task 174 artifacts committed ebf29ad. Task 175 dispatched to impl-175 (Sonnet), base ebf29ad: search CLI via boot.AbrirIndice/PrepararBusca, --cache-dir/--log-level, loggerDeCLI, Config.LogLevelExplicito, M4 measurement.
Task 175 impl commit 8d3626e (DONE; verify 14/14 after an errcheck fix on the brief's `defer inv.Close()`). Measured antes 1.6-1.7 s / depois warm 190-235 ms on vault_5000 - not the M4 figures the brief quoted (those were another vault); ratio holds. rev-175 (Sonnet) dispatched on review-ebf29ad..8d3626e.diff.
Task 175 review: approved, no findings. Orchestrator removed impl-175's leftover detached worktree antes175-wt (scratchpad). Task 175: complete (8d3626e).
Task 175 artifacts committed 6b70721. Task 176 dispatched to impl-176 (Sonnet), base 6b70721: index/inspect via boot.AbrirIndice, origin in index --json, flags.go (flagsDeCofre/flagsDeCache), two commits.

[2026-09-06 16:45 orchestrator] The entries below (Task 176 through the plan's close) were appended by mistake to the pointer file .superpowers/sdd/2026-09-02-topografia-e-limpeza/progress.md; migrated here verbatim, order preserved.
...
- 2026-09-06 16:35 impl-final DONE: cd61624 fix(cli) doctor --debounce-ms removed (+test, mutation RED pasted), ada4897 fix(mcpsrv) tags jsonschema descriptions (+served-schema test, mutation RED pasted), b3ee480 docs final pass (A1-A8, B-F3, B-N1, B-N5, B-Stale; 34 files +260/-109). Full verify.ps1 green three times (pasted). audit_reports -Task final: 0 findings on the report (re-run by orchestrator), 14 old-ledger known. Orphans gate not run (ruling above). Note: 4627ea7 (sdd artifacts) is the orchestrator's commit inside the range.
- 2026-09-06 16:37 Scoped re-review dispatched to rerev-final (Sonnet), package review-5c613d5..b3ee480.diff. Output: review-final-round1.md.
- 2026-09-06 16:42 rerev-final APPROVED (review-final-round1.md): no new findings; F1/F2 mutations reproduced independently; 8 cited symbols exist; ESTRUTURA tree matches ls; commit hygiene clean.
- Final review: complete (cd61624, ada4897, b3ee480). Plan 2026-09-02-topografia-e-limpeza: ALL TASKS COMPLETE. Branch master, range 6c5d1f1..b3ee480. Parked for the owner: rev-final N2/N3/N4 and the out-of-scope list in the closing message. Plan workspace kept (tracked in git, not scratch).
- 2026-09-08 Investigação do `Server transport closed unexpectedly` relatado pelo dono. Medido, não inferido: (a) a desconexão de 2026-09-07T17:45:15 é o INSTALADOR, que mata processos para trocar o binário (`OPERACAO.md:1970`); mtime do exe 17:45:03, 12 s antes; (b) daemon PID 42856 pediu `reason=idle` às 19:30:36 e seguia vivo 20 h depois — 274 MB, 0 s de CPU em 3 s, 26 threads em `Wait,UserRequest`; (c) `cleanupSocketFile` falhou 20+ vezes desde 2026-09-01 com 1920, derrubando toda ponte para o modo em processo; (d) dois processos gravando o mesmo `inverted_cache.gob` (PIDs 7920 e 42628), provado por rename duplicado com ms diferentes; (e) 960 `.lock` e 3 `.sock` órfãos em `%LocalAppData%\gobsidian\run`. Dois reprodutores escritos: nenhum produz o par de campo (dial 10022 / remove 1920) — órfão normal dá 10061 e `os.Remove` funciona. Troca de binário a quente medida: sobrescrever falha, **renomear funciona** com o processo vivo. Delve não fecha a conta: binário com `-s -w`, `could not find goroutine array`. Processos 42856 e 42628 encerrados com autorização do dono.
- 2026-09-08 Spec 4390a1b (`docs/superpowers/specs/2026-09-08-instalador-e-encerramento-design.md`), D-01..D-13 com a alternativa descartada em cada uma. D-13 abre a SEGUNDA exceção nomeada à RNF-30: `net/http` só em `internal/selfupdate`, host em constante literal, no molde da exceção `"unix"` de 2026-08-05. Plano 1 fb47418 (`2026-09-08-encerramento-e-logs.md`), tasks 189–194.
- Ruling (Task 189): o guarda-chuva vai nos TRÊS pontos de saída (`serve`, ponte, daemon), não só no daemon — `internal/boot/busca.go:209` já descrevia o mesmo buraco em `serve` por escrito. Escopo ampliado de propósito; custo se errado: um guarda a mais num caminho que nunca falhou.
- Ruling (Task 189): orçamento 6 s, não 8 — `test_orphans.ps1:6-9` fixa que a janela do harness (8 s) tem de ser MAIOR que o guarda-chuva; igualar cria corrida na borda. Nenhum script de gate mudou.
- Task 189: complete (dd1e638). Prova de mutação COMPILANDO: `time.After(orcamento)` → `time.After(time.Hour)`, teste falha com "o subprocesso passou do prazo sem sair". A primeira tentativa de mutação foi INCONCLUSIVA (não compilava) e o teste foi reescrito para observar o processo de fora, com prazo — sem prazo, o mutante travava em vez de falhar.
- Task 190: complete (847a84e). Duas mutações compilando: WARN→INFO e classificar só pelo erro final. O RED revelou que `startErr` é *timeout* no caso de versão divergente — a verdade está no primeiro dial —, e a classificação passou a olhar todos os erros do caminho.
- Task 191 + rotação (194): complete (b205d57). Três mutações compilando. Gate de órfãos rodado depois da mudança (5 ciclos, `reason=idle` lido como antes) para provar que os campos novos não quebraram o parsing.
- Task 192: complete (94a3c14). Plano B por `rename`, sem tratar errno — classificar por número é o que o comentário de `Listen` já proíbe. Testes num arquivo interno porque `ipc_test.go` é pacote externo. Duas mutações compilando.
- Task 193: NÃO EXISTE, e está escrito no plano. `doctor` já reporta o modo (`daemon.go:107`) e o cache-dir (`checks.go:212`); uma checagem nova seria duas contas da mesma regra. O que a spec pede e falta de verdade — "há mais de um processo servindo este cofre?" — exige enumeração de processos e vai para o plano 2, que a constrói de qualquer forma. Redução de escopo DECLARADA.
- Task 194 (docs): complete (68a600b). Duas dívidas registradas como ABERTAS em `ESTADO.md`: a causa do encerramento pendurado (sabe-se que está fora de `Shutdown`; qual das três esperas, não) e o estado de socket 10022/1920 (não reproduzido). Anteparo, não conserto — e escrito assim.
- `verify.ps1` completo, verde, rodado três vezes ao longo do plano 1 (a primeira e a segunda reprovaram em `golangci-lint` sobre código de teste novo, corrigido nas duas).
- 2026-09-08/09 Plano 2 (`2026-09-08-instalador-no-binario.md`, 4f7560f), tasks 195–203. Todas completas. Commits: aa5490c (195+196 presença e trava global), 8b77abc (197 limpeza), a1d5307 (199 selfupdate + 2ª exceção RNF-30), 55a33ef+a5528d3 (202 bootstraps e remoção dos instaladores), 7c0a74a (200 manifesto/PATH/sequência), dab69e4 (198 hosts), 8f00477 (201 subcomandos e D-11), ff1a526 (203 CI). `verify.ps1` completo verde antes de cada um.
- Ruling (195): presença por TRAVA DE KERNEL, não por enumeração de processos. Go não tem listagem portável e o projeto proíbe `if runtime.GOOS ==`; `internal/daemon/trava.go` já trava a faixa em `1<<62`, longe dos dados, escolhida assim para o `doctor` poder ler o PID de dentro do arquivo travado. Custo se errado: um pacote a mais importando `daemon`.
- Ruling (197): a limpeza remove `.lock` mesmo com `trava.go` dizendo que ele nunca é removido. Não é contradição — aquela regra é sobre OPERAÇÃO, onde a janela entre remover e recriar era a origem das corridas; a limpeza roda sob a trava global, sem processo no ar, e só sobre travas que ela mesma verificou livres. Escrito no comentário.
- Ruling (199): a regra de literais de URL do `netcheck` NÃO vale em `_test.go`. O teste que prova que um host proibido é recusado precisa escrever um host proibido, e a RNF-30 é sobre o binário entregue. A metade dinâmica fica com `selfupdate.ValidarHost`, com teste. Custo se errado: um host escrito num teste que ninguém disca.
- **Incidente, 2026-09-08, causado e desfeito na mesma sessão.** A primeira versão do teste de D-11 chamou a instalação REAL. Ela leu o registro do Obsidian, escolheu um cofre, detectou seis hosts de IA e reescreveu a configuração dos seis para apontar para o BINÁRIO DE TESTE, além de pôr o diretório no PATH do usuário e gravar manifesto. Duração do subteste: 25,6 s — foi o que denunciou. Desfeito por inteiro: os três JSON restaurados byte a byte dos `.gobsidian-backup` que `hosts.Fundir` grava, `claude`/`gemini` desregistrados pelos próprios CLIs, entrada removida do `mcp.json` do VS Code, PATH limpo, manifesto e binário de teste apagados. `doctor` depois: daemon respondendo, seis servidores MCP do dono preservados. Registrado em `docs/ARMADILHAS.md` com os sintomas que revelam a classe. Teste corrigido: `rodarInstalacaoFn` vira variável de pacote e o teste a substitui por um gravador — 0,24 s.
- Correção de afirmação não verificada: a spec, o plano e `ESTADO.md` diziam "3 `.sock` órfãos de cofres que não existem mais". Medido lendo `search.CacheHeader.VaultPath` de cada cache: as quatro chaves são Estudo, Jurisprudência, Oral e Revisão, e **os quatro cofres existem**. Socket sem daemon rodando é estado normal. Os três documentos foram corrigidos em 8b77abc.
- Correção de grafo: a primeira redação do bloco novo do CLAUDE.md dizia `selfupdate → config`. `GOOS=windows go list -f '{{.Imports}}'` diz **folha**. Corrigido antes de commitar, com a correção registrada no próprio parágrafo.
