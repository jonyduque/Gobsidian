...
**4. Em 2026-09-01 o requisito passou a nomear o cenário de cache.** A redação
anterior dizia "nos estados `pronto` e `servindo`" e não dizia nada sobre o
cache, e pela letra os números de cache frio a violavam.

O alvo vale **com cache válido**, que é o regime permanente: o servidor passa
nele praticamente toda a vida útil, e é o que uma sessão real encontra.

**Durante a reconstrução do índice não há requisito, e isso é deliberado.**
Medido nos cinco cofres com o cache apagado, o `servindo` fica de 5× a 12× acima
do alvo — Estudo 58 → 706 MB, TJSP 127 → 1.180 MB. Transformar esse pico em
requisito exigiria um alvo, e o único número honesto disponível seria "o pior que
já medimos" — um requisito que ninguém pode violar por construção descreve, não
exige, e vira ruído na tabela. **Decisão do dono em 2026-09-01.** O pico está nos
limites conhecidos de `OPERACAO.md`, com os números por cofre.
...
>
> **Remedição de 2026-08-06 no cofre real, mesmo dia: RNF-02 melhora mas não atinge o teto.** O cofre real citado no ponto 2 cresceu desde então — 4.165 notas hoje, não mais 3.149. Cinco partidas sem cache: **1230–1389 ms**, na mesma faixa do que já estava registrado. Cinco partidas com cache: **397–547 ms**. **RNF-02 segue não atingido** no cofre real — cerca de 3× mais rápido que antes, mas acima do teto de 300 ms, ao contrário do que o cofre sintético do parágrafo anterior sugeria. A hipótese mais provável para a diferença: `Index.VerifyFreshness` faz uma varredura do cofre inteiro (só `Stat`, sem parse) de forma sequencial antes de aceitar o cache, e essa varredura em disco sincronizado por nuvem tem uma latência por arquivo que um disco local não tem — a mesma varredura, dentro de `Build`, roda em paralelo com o parse e fica menos visível no total. Não investigado a fundo nesta tarefa; fica registrado como o próximo alvo de otimização, não escondido atrás do número do cofre sintético. Detalhe completo em `docs/OPERACAO.md`.
