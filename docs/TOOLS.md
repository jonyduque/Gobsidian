...
**Notas.** Negrito só conta **sozinho na linha**: ênfase no meio de um parágrafo não é título, e aceitá-la encheria a resposta de ruído. E o texto precisa de **pelo menos 5 caracteres**, piso que saiu de medição: nos cofres reais o ruído tinha um nome só, `**V:**` — marcador de verso de flashcard, sozinho na linha —, e ele respondia por 14.940 dos 14.940 candidatos de um cofre e por 14.961 dos 22.180 de outro. O piso zerou o primeiro, cortou 65% do segundo, e custou 1,6% num terceiro, onde o que caiu foi `Não.`, `SIM.` e `22` — respostas de flashcard também.

Dois filtros foram medidos e **recusados**: "termina com dois-pontos" descartaria `Jurisprudência:` e `Constituição Federal de 1988:`, que são rótulos de subseção legítimos; e "repetido em muitas notas" marcaria `1. Objetividade jurídica` (152 ocorrências) e `3. Sujeito ativo` (127), que são a estrutura que um tratado repete a cada capítulo — repetição ali é sinal de livro estruturado, não de ruído. Detecção acontece **fora de bloco de código cercado**, pela mesma máquina de cercas de `ExtractHeadings` — duas máquinas de cerca divergiriam, e a divergência apareceria como hierarquia falsa dentro de um bloco de código, que é o pior lugar para ela aparecer numa nota que documenta Markdown.
