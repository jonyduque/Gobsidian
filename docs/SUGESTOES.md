...
|---|---|---|
| **P5** | Remover `Hits`; `results` fica | Apagar o campo `Hits` de `SearchResponse` (`service/search.go:97`) e as três atribuições (`:176,220,315,338,370`). Atualizar `TOOLS.md:62`, que hoje diz *"contendo `results` (e `hits`)"*. Grep em goldens e CLI antes — reprovar a suíte por um campo que ninguém lê seria custo sem ganho. — **feito em 2026-09 (Task 179)** |
