...
- [x] G1.0 — `tools/sondahost`: o servidor de diagnóstico, com instrução de registrar, reiniciar o host e ler a saída. Ele sonda, em cada diretório candidato: `listen`, `lstat` e `dial` de socket próprio; `dial` num socket deixado por processo High; trava `LockFileEx` num arquivo compartilhado com um processo externo; e criação de diretório novo na raiz de `%LOCALAPPDATA%`, conferindo se cai no caminho real ou em `LocalCache`. **Feito em 2026-09-14:** `tools/sondahost` (modos `preparar`, processo do host, `ler`, `limpar`). Rodada no Claude Desktop 1.52386.6: três processos do host, todos Medium, em job `0x3C00`, sem identidade de pacote.
- [x] G1.1 — `dial` cruzado em `%USERPROFILE%\.gobsidian\run`: socket criado por processo High, conexão do processo do Desktop. Medido só em `Temp` até agora. **Medido: passa.** `%USERPROFILE%\.gobsidian-sondahost\run` — socket próprio `listen/lstat/dial` ok, e `dial` no socket criado pela shell elevada ok, nos três processos. `Temp` também passou; o controle `%LOCALAPPDATA%\gobsidian\run` reprovou de novo com `1920`/`10022`.
- [x] G1.2 — Exclusão mútua por `LockFileEx` num arquivo em `%LOCALAPPDATA%\gobsidian\run`, entre um processo High e um do Desktop. Arquivo comum funciona lá; trava de kernel **não foi medida**. Decide se as travas podem ficar onde estão (G2). **Medido: passa, com uma condição.** A trava tomada pela shell elevada foi recusada nos três processos do Desktop, e a trava livre foi tomada — a primitiva funciona e a exclusão mútua vale entre os contextos. A condição é G1.3: isso foi num diretório que já existia no caminho real.
- [x] G1.3 — Diretório de primeiro nível criado pelo processo do Desktop numa máquina onde ele ainda não existe: cai em `LocalCache`? Arquivo na raiz caiu. Se diretório também cair, uma máquina onde o Desktop é o primeiro a rodar dividiria o **cache** entre dois lugares, e a raiz do cache (`config.RaizDoCache`) também precisa mudar. **Medido: desvia.** Os três diretórios novos que o processo do Desktop criou na raiz de `%LOCALAPPDATA%` não existem no caminho real; cada um apareceu em `%LOCALAPPDATA%\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local\`. Numa máquina onde o Desktop é o primeiro a criar `%LOCALAPPDATA%\gobsidian`, travas, log, presença e cache iriam para uma cópia privada que nenhum outro host enxerga.
- [x] G1.4 — Registrar as respostas em `docs/ESTADO.md`, com data e versão do Desktop. **Feito:** `docs/ESTADO.md`, dívidas.

### G2. Tirar o socket de `%LOCALAPPDATA%` — feito em 2026-09-14: o diretório de runtime inteiro

**Revisão depois de G1 (2026-09-14). Decisão do dono, no mesmo dia: caminho 1, o diretório de runtime inteiro.** G1.1 e G1.2 passaram, mas G1.3 mudou a conta: diretório de primeiro nível criado pelo processo do Desktop em `%LOCALAPPDATA%` é desviado para o `LocalCache` do pacote. "Mover só o socket e deixar as travas em `%LOCALAPPDATA%\gobsidian\run`" só é seguro onde esse diretório já existe no caminho real — e é exatamente o que a trava precisa garantir, não supor. Três caminhos:

1. **Recomendado — o diretório de runtime inteiro vai para `%USERPROFILE%\.gobsidian\run`:** socket, travas, log, presença e `instalacao.lock` juntos, um lugar só, fora do escopo medido de virtualização. É G2.3: a transição com binário antigo vivo exige a ponte nova recusar subir daemon se a trava **antiga** estiver tomada. O cache fica em `%LOCALAPPDATA%\gobsidian`; o risco que sobra é uma máquina onde o Desktop cria esse diretório primeiro ter o cache dividido em dois — reconstrução em dobro, não corrupção, porque são arquivos diferentes. **Não medido:** exclusão mútua de trava em `%USERPROFILE%` entre os contextos; entra na aceitação G2.5.
2. **Só o socket muda** (o texto original abaixo), e o instalador passa a criar `%LOCALAPPDATA%\gobsidian` fora do Desktop. Quem instala sem o instalador e usa só o Desktop fica com as travas numa cópia privada, sem aviso.
3. **Runtime e cache vão para `%USERPROFILE%\.gobsidian`.** Fecha a classe inteira; custa migrar os caches existentes (tamanho não medido) e mudar `config.RaizDoCache`, a limpeza e o `doctor`.
...
- [x] G2.1 — `ipc.DiretorioDeSockets()` (nova) e `ipc.RuntimeDir()` (travas, log, presença). No Unix as duas devolvem o mesmo diretório de hoje: o defeito é do Windows. Código de plataforma em `ipc_windows.go`, nunca `runtime.GOOS`. **Feito em 2026-09-14, de outra forma:** pela decisão do dono (caminho 1) não houve divisão entre diretório de sockets e de travas. O diretório de runtime INTEIRO mudou, e como tudo deriva de `ipc.RuntimeDir`, a mudança é `runtimeDirDoSistema` no Windows: `%USERPROFILE%\.gobsidian\run`. Unix sem mudança. Mutação: de volta para dentro de `LOCALAPPDATA`, `TestRuntimeDirDoSistemaFicaNoPerfilNoWindows` reprova; a primeira tentativa desse mutante não compilava e foi refeita.
- [x] G2.2 — `SocketPath` passa a usar `DiretorioDeSockets`; `lockPath` e o log do daemon continuam em `RuntimeDir`. Teste: no Windows, o socket fica fora de `%LOCALAPPDATA%` e a trava continua dentro. **Não se aplica:** sem a divisão de G2.1, `SocketPath`, `lockPath` e o log continuam derivando da mesma raiz, que é a que mudou.
- [x] G2.3 — Se G1.2 **reprovar**, tudo muda de lugar junto, e a transição com binário antigo vivo precisa de caminho próprio: a ponte nova recusa subir daemon se a trava **antiga** estiver tomada. Não escrever antes de a medição mandar. **Feito, com o sinal corrigido:** um daemon em regime não segura trava de inicialização nem de escuta — só enquanto sobe. O que ele segura a vida inteira é a presença. `instalar.DaemonAnteriorDoCofre` procura presença viva de `daemon` do mesmo cofre no diretório antigo (`ipc.DiretorioDeRuntimeAntigo`, vazio no Unix e sob o desvio dos testes), e a ponte serve em processo com `motivo=daemon-de-versao-anterior`. Mutações: daemon de qualquer cofre contando e checagem desligada na ponte reprovam os testes; restaurados, passam.
- [x] G2.4 — `instalar` (limpeza) passa a varrer os dois diretórios; sockets órfãos no diretório antigo viram lixo coberto pela varredura que já existe. **Feito:** `instalar.Limpar` varre também o diretório antigo. Mutação: sem a varredura, `TestLimparVarreODiretorioAntigo` reprova.
- [x] G2.6 — Achado ao rodar o `doctor` do G7 nesta máquina: o binário novo lia presença só no diretório novo, e os 28 processos v1.8.1 vivos, que registram no antigo, saíam todos como "sem presença". O mesmo buraco estava em `encerrarProcessos`: um `install` feito pelo binário novo não encerraria os v1.8.1 antes de trocar o executável que eles mantêm aberto. **Feito em 2026-09-14:** `instalar.VivosComAnterior`, usado pelo `doctor` e por `encerrarProcessos`. Mutações: com o diretório antigo ignorado, `TestInstalarEncerraProcessoDoDiretorioAntigo` reprova com "encerrou [], esperado o PID registrado no diretorio antigo" e `TestVivosComAnteriorVeOsDoisDiretorios` com só a presença v2; com `encerrarProcessos` de volta a `Vivos`, o primeiro reprova igual. Restaurado, passa. Depois: o `doctor` real lista os 28 processos v1.8.1 com papel e cofre, e sobra sem presença só o pid do v1.5.1.
- [ ] G2.5 — Aceitação na máquina do dono: daemon v1.8.1 vivo, binário novo no Desktop — **um** daemon por cofre, nunca dois. É o cenário de versões mistas, que `scripts/test_orphans.ps1` não cobre. **Pendente:** exige instalar o binário novo nesta máquina e reiniciar os hosts — entra em G6.
...
- [x] G7.1 — `doctor` (seção de hosts, via `instalar`) lista entradas do gobsidian nos configs de host cujo `command` não é o binário instalado, com a versão de cada um. Medido nesta máquina: Claude Code em v1.5.1 (`C:\Program Files\gobsidian`), Desktop em v1.8.1. Teste com config de fixture. Não reescreve config nenhum: relata. **Feito em 2026-09-14, com um desvio:** `instalar.EntradasDeOutroBinario` (comparação por `os.SameFile`, a mesma de `EstaInstalado`) e a seção "binario dos hosts" do `doctor`. A versão sai da presença de um processo vivo daquele executável (`instalar.VersaoDoExecutavel`), e **não** de rodar o binário: um executável de outra instalação, rodado pelo `doctor`, escreveria log e runtime do jeito da versão dele. O custo é o caso medido — o v1.5.1 não tem presença e sai como "versao nao medida"; o caminho dele aparece, e a lista de processos sem presença (G8.3) mostra que está rodando. Mutações: sem pular o instalado, `gobsidian-estudo` entra na lista; tratando comando sumido como o instalado, `gobsidian-revisao` some; versão sem conferir o executável, o novo sai `v1.9.0` em vez de `v1.8.1` e o antigo ganha versão que não tem. Restaurado, passa.
...
- [x] G8.1 — `Presenca` ganha `modo`: `daemon`, `ponte` ou `em-processo`. A ponte regrava a própria presença, sob a trava que já segura, logo depois de decidir. Até decidir, `modo` fica `decidindo`, e o `doctor` mostra assim. **Feito em 2026-09-14, com um desvio:** o estado `decidindo` e a regravação não foram necessários. Os três pontos que registram presença — `serveEmProcesso`, `servePonteRemota` e o daemon — já rodam depois da decisão, então o modo entra no próprio registro (`prepararProcesso(log, papel, modo, cofre)`). As constantes moram em `internal/instalar/processos.go`. Mutação: sem `Modo` no JSON, `TestPresencaGravaOModo` reprova com `modo lido = ""`; restaurado, passa.
- [x] G8.2 — O aviso de duplicidade conta **só gravadores** (`daemon` e `em-processo`) e nomeia os PIDs gravadores extras. Pontes aparecem na lista, sem aviso. Teste com presenças de fixture: um daemon e três pontes não avisam; um daemon e um em processo avisam, com o PID do em processo. **Feito:** `analisarGravadores` no `doctor`, e `instalar.GravaCache` como a conta única de quem grava. Processo sem modo (versão anterior) aparece à parte e não conta. Mutações: ponte contada como gravador derruba `TestGravaCacheSoDaemonEEmProcesso` e dois subtestes de `TestAvisoDeDuplicidadeContaSoGravadores`; processo sem modo contado como gravador derruba o terceiro subteste. Restaurados, passam.
- [x] G8.3 — Enumerar processos `gobsidian.exe` do sistema e listar os que não têm presença, com o executável. Código de plataforma atrás de build tag: Windows primeiro; nas outras plataformas a linha diz "não verificado nesta plataforma", em vez de fingir que não há nenhum. **Feito:** `instalar.ProcessosDoSistema` (`processos_windows.go`, Toolhelp32 e o caminho do executável; `processos_other.go` devolve `ErrProcessosNaoVerificados`) e `instalar.SemPresenca`. O teste do Windows sobe uma cópia do binário de teste chamada `gobsidian.exe`, que não registra presença, e confere PID e caminho. Mutação: `SemPresenca` sem tirar os registrados derruba o teste puro e o do Windows. Rodado nesta máquina com o binário novo: os 28 processos v1.8.1 aparecem como `sem modo registrado (versao anterior)`, e o `doctor` achou `pid 46888  C:\Program Files\gobsidian\gobsidian.exe`, sem presença.
