...
desenho acima deixou duas regras vivendo só como código em
`cmd/gobsidian/serve.go`, sem teste:

1. **A decisão de código de saída sai numa função pura,
   `shutdownExitCode(err error) int`.** `runServe` termina em `os.Exit` por
   desenho — `os.Exit` não volta, e é isso que torna a decisão intestável
   enquanto ela vive dentro da função. Encerramento normal são três erros, não
   um: `context.Canceled` (vem do lifecycle), `io.EOF` e `io.ErrClosedPipe`
   (podem vir do SDK, que detecta o fim do stdin por conta própria). As duas
   detecções correm, e qual vence decide qual valor chega. Comparar com
   `errors.Is`, nunca `==`: o SDK entrega o erro embrulhado.
2. **A drenagem de `lateErr` vem depois de `lc.Wait()`, não antes.** A etapa
   in-flight pode ter sido abandonada por estouro de orçamento com sua
   goroutine ainda a caminho do canal; `Wait` é o último ponto em que esperar
   por ela é de graça. A drenagem continua não-bloqueante, e o descarte
   residual é deliberado.
...
Três propriedades do script não são negociáveis, e cada uma existe porque a
...
3. **A varredura final se restringe aos PIDs que este script lançou.** Varrer
   por nome mata o `gobsidian serve` que o desenvolvedor deixou aberto noutro
   terminal e o conta como órfão.
...
for ($i = 1; $i -le $Cycles; $i++) {
    ...
    # O PID do filho so existe depois que o host o cria; ler cedo demais e
    # corrida, nao falha. Poll com timeout — e um ciclo cujo filho nunca
    # apareceu e ERRO, nunca passagem silenciosa.
    ...
    # Mata o HOST, nao o filho, e sem /T. E a morte do host que fecha o pipe
    # e dispara o EOF. Matar a arvore inteira provaria so que taskkill sabe
    # matar processos.
...
# Varredura final restrita aos PIDs que ESTE script lancou: pega o caso do
# host morto entre o poll do PID e o taskkill, sem matar um servidor que o
# desenvolvedor deixou aberto noutro terminal.
...
# Desconta os ciclos ja contados como LaunchFailures: um ciclo que nunca
# lancou nada nao tem motivo para registrar, e cobra-lo duas vezes so
# embaralha o diagnostico.
