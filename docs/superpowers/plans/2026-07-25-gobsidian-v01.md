...
- **Entrada descartada em silêncio.** `vault.SkippedEntries()` existe porque uma nota que está no disco e não entra no índice fica inalcançável *e* indiagnosticável. O filtro descarta por projeto; portanto **conte o que descartou, por motivo**. A Task 32 expõe esses números, e contador que só nasce lá chega tarde demais.
  ...
  **Correção da revisão de 2026-07-28.** Os motivos são exatamente quatro e não podem ser agregados num contador só: `chmod` (evento de permissão), `outside_vault` (`vault.Canonicalize` recusou o caminho), `excluded` (`vault.Classify` devolveu `ClassExcluded`), `unknown_op` (nenhum bit de `Create`/`Write`/`Remove`/`Rename` presente). O incremento acontece **onde a decisão é tomada** — `filter` devolve o motivo junto com o booleano. Reconstruir o motivo no chamador é como um contador agregado nasce de novo. A entrega original agregou os quatro num campo só; a Task 37 desfaz isso.
...
- Produces: `vault.Classify`, `vault.Classification`; `watcher.Event`, `watcher.Op`; `watcher.New(*vault.Vault, *slog.Logger) (*Watcher, error)`; `(*Watcher).Run(ctx) error`; `(*Watcher).Close() error`; contadores de recebidos e de descartados **por motivo**

> **Correção da revisão de 2026-07-28.** Este contrato declarava `(*Watcher).Events() <-chan Event`. O método não existe: a Task 28 passou a consumir o canal internamente e o acessor nunca foi criado. Contrato que declara método inexistente é o mesmo defeito do schema que prometia `fields` e o descartava — foi removido daqui em vez de reintroduzido no código, porque ninguém fora do pacote precisa do canal cru.
...
- **A janela vem da configuração, não de uma constante.** `config.Config.DebounceMS` já existe, já é validado, e já tem companheiro `DebounceMSSet` para distinguir "omitido" de "definido como zero". O padrão é 250 ms. Não recodifique 250 em lugar nenhum além do default da config.
  ...
  **Correção da revisão de 2026-07-28.** A entrega original recodificou 250 ms como fallback dentro do debouncer (`if window <= 0 { window = 250 * time.Millisecond }`), o que apagava em silêncio um `--debounce-ms=0` explícito. **Decisão fechada:** zero deixa de ser valor aceito. `validateDebounceMS` passa a exigir `n >= 1` e a recusa mora na config, não no watcher — o debouncer não pode ter fallback nenhum. O motivo da recusa: sem coalescência cada evento vira um lote de um caminho só, e a correlação de rename da Task 31, que exige uma remoção **e** uma criação no **mesmo** lote, para de detectar qualquer rename. Servidor sem debounce não é configuração que se possa pedir por engano. Implementado pela Task 38.
...
- **Um teste que não pode falhar é pior que teste ausente.** Um teste que dispara overflow e verifica que o índice está certo passa mesmo sem reconciliador, se os eventos normais tiverem chegado do mesmo jeito. **O teste precisa perder eventos de verdade:** modifique o cofre com o watcher pausado ou desconectado do canal, injete o overflow, e só então verifique. Prove por mutação: desligue o reconciliador, confirme que o teste reprova nomeando a divergência, restaure.
  ...
  **Correção da revisão de 2026-07-28. Este aviso foi dado e ignorado.** A entrega original deixou o watcher rodando e consumindo eventos normalmente, então o pipeline comum aplicava as três mudanças e a reconciliação nunca era medida. Medido: com o corpo de `Reconcile` substituído por `return`, `TestOverflowReconciliationFull` passa em 2,8 s. O requisito é P0 (RF-05) e estava com cobertura zero.

  A cobertura exige **três** testes distintos, porque são três mecanismos, e nenhum dos três pode ligar o watcher inteiro:
  1. `TestReconcile_CorrectsLostEvents` — **nenhum watcher rodando**. Constrói o índice, muda o cofre por baixo (um modificado, um removido, um criado), chama `Reconcile` direto, afirma os três.
  2. `TestApply_ReconcileSignal` — `Apply` rodando com o canal de entrada **vazio e nunca alimentado**. Muda o cofre, manda um `struct{}{}` em `reconcile`, afirma o índice corrigido. Prova que a correção veio do sinal, não de evento.
  3. `TestRun_OverflowSchedulesExactlyOne` — injeta N erros `ErrEventOverflow` e afirma que exatamente **um** token chega ao canal `reconcile` e que o contador subiu N.
...
- Como o `fsnotify` v1.10.1 sinaliza overflow na versão fixada — `ErrEventOverflow` no canal de erros, outro valor, ou nada em alguma plataforma? **Verifique no código da dependência e reporte**; se alguma plataforma não sinalizar, isso é uma lacuna que precisa ficar registrada, não resolvida por suposição.
  ...
  **Resposta medida na revisão de 2026-07-28, para não ser re-perguntada.** No `fsnotify` v1.10.1, `ErrEventOverflow` é emitido em exatamente dois lugares: `backend_inotify.go:398` (Linux) e `backend_windows.go:582` (Windows). O backend kqueue — macOS e BSD — **não o emite em nenhuma circunstância**. Em macOS a reconciliação por overflow nunca dispara, e o único anteparo contra evento perdido é a reindexação no boot. **Isso é lacuna registrada, não resolvida:** fica em `ARCHITECTURE.md` §5.3, num comentário sobre `Reconcile`, e no ledger. Não invente heurística de detecção. O relatório original da Task 30 afirmava "Windows e OSX", o que é falso.
...
**Correções da revisão de 2026-07-28.** Quatro exigências que o texto original deixou implícitas e que por isso não foram cumpridas:

- **Nada é aberto antes de saber que é nota local.** A entrega original chamava `vault.ReadAll` para **todo** caminho do lote que existisse no disco — anexos inclusive, placeholders somente-nuvem inclusive. `vault.ReadAll` é `os.ReadFile` puro, e o comentário dele diz que bloqueia indefinidamente enquanto o cliente de nuvem hidrata. Isso viola as duas regras fechadas da Task 29: *anexo é indexado por nome, nunca lido* e *arquivo somente-nuvem não é aberto*. **Só é elegível a hash o caminho com `vault.Classify(p) == vault.ClassNote` e `!vault.IsCloudOnly(v.Abs(p))`.** Todo o resto vai para `nonRenames` sem nunca ser aberto.
- **`CorrelateRenames` recebe `ctx`** e o repassa a `vault.ReadAll`. `context.Background()` dentro dela é violação direta da regra de `ctx` do projeto.
- **Cada caminho é lido no máximo uma vez e aparece na saída exatamente uma vez.** A entrega original tinha duas passagens de leitura e devolvia caminho duplicado para arquivo vazio — medido: um `empty.md` criado saía como `[empty.md empty.md]`, fazendo `Replace` rodar duas vezes e inflando o contador `processed`.
- **Correlacionar não é só devolver o par.** O teste tem de afirmar (a) `From` e `To`, (b) que `Backlinks` traz os candidatos do caminho antigo, (c) que uma nota **com BOM** correlaciona, e (d) que nenhum arquivo do cofre foi escrito, por *snapshot de mtime de todo o cofre antes e depois* — não por argumento em prosa.
...
**Limite a notas.** Registre a limitação em `docs/ARCHITECTURE.md` §5.3 e no relatório.

**Decisão de 2026-07-28 sobre `index.MoveNote`.** A entrega original acrescentou `index.MoveNote(oldPath, newPath)` — API que este contrato não previa — e fez o `Apply` aplicar o rename correlacionado **só** por ela, sem `Remove` do caminho antigo nem `Replace` do novo. Ela fica, por decisão explícita: rename de diretório move N notas de uma vez, e é o caso em que evitar N reprocessamentos completos do grafo passa a importar. Mas ela entra no contrato com três dívidas a pagar (Task 36): equivalência provada estrutura por estrutura contra `Remove`+`Replace`, atualização de `Size`/`ModTime` a partir do caminho novo, e um teste de integração — hoje, com `MoveNote` reduzida a no-op, **todos** os testes do pacote `watcher` passam. Um anexo renomeado continua aparecendo como remoção mais criação, e os embeds para o caminho antigo ficam quebrados até a próxima reindexação — que é o comportamento de hoje, não uma regressão.
...
## M2.1 — Correções da revisão do M2 (Tasks 33–42)

A revisão de 2026-07-28 das Tasks 27–32 encontrou três defeitos Critical, cinco Important e um conjunto de higiene. Nenhum deles é hipotético: cada um foi reproduzido por mutação ou por sonda, e a evidência está registrada na tarefa que o corrige.

**As Tasks 33 a 42 são estritamente sequenciais.** Não é preferência: 33, 34, 37, 38 e 40 tocam arquivos em comum (`internal/watcher/apply.go`, `watcher.go`, `debounce.go`, `overflow.go`), e executá-las em paralelo produz conflito garantido. Rode uma, revise, feche no ledger, comece a próxima.

**Cada seção abaixo é autocontida.** Traz onde encaixa, as decisões fechadas que a vinculam, a evidência medida do defeito, as armadilhas já pagas que se aplicam, verificações além dos passos, regras de execução e contrato de relatório. O brief extraído basta para executar — não é preciso injetar contexto acumulado no prompt.

**Três decisões foram tomadas antes de escrever estas tarefas, e não devem ser re-litigadas:**

1. **`--debounce-ms=0` passa a ser recusado na configuração** (`n >= 1`), em vez de ser aceito e silenciosamente virar 250 ms no watcher. Task 38.
2. **`index.MoveNote` fica**, e paga as dívidas que contraiu ao entrar fora do contrato. Task 36.
3. **Os contadores de descarte são publicados desdobrados por motivo** em `vault_stats`, e não só agregados. Task 37.
...
Acrescente o comentário que amarra a lacuna de plataforma ao ponto de uso. **Ele vai no doc comment de `Reconcile`, depois da primeira frase, não antes dela:** o `revive` exige que o comentário de função exportada comece com o nome da função, e pôr a nota de plataforma na frente reprova o lint. Esta instrução dizia "no topo de `Reconcile`" e foi transcrita ao pé da letra, o que custou um achado de `revive` na revisão de 2026-07-29.
...
**Rode `golangci-lint run ./internal/... ./cmd/...` antes de commitar.** As Tasks 33 a 42 fecharam com 22 achados acumulados — 18 `errcheck` em arquivos de teste novos e 4 `revive` — e nenhum dos dez relatórios mencionava ter rodado o lint. `go vet` não pega `errcheck`, e `verify.ps1` não roda `golangci-lint`.
...
## M3 — Busca (Tasks 43–50)

Índice invertido, BM25, analisador de português com indexação dupla, `vault_search`, e o cache de índice em disco. Fecha a Q3 do PRD §11 **por medição**, não por escolha.

**Estritamente sequenciais.** A 44 produz os tokens que a 45 indexa, que a 46 pontua, que a 47 recorta, que a 48 serve, que a 49 persiste. A 43 vem antes de todas porque conserta um defeito de parser que a busca amplifica.

### As decisões que já estão fechadas, e não devem ser re-litigadas

**D9 do PRD — indexação dupla.** Cada token entra no índice na forma crua normalizada e, quando a redução produz algo diferente, também na forma reduzida, **ambas apontando para a mesma posting list**. A consulta casa qualquer uma; a busca por frase exata usa só a forma crua. Não troque por stemmer completo: Snowball e RSLP fundem termos de arte que só se distinguem pelo sufixo, e este cofre é técnico.

**N6 do PRD — sem embeddings.** Busca é léxica na v1. RF-26 é P2 e não entra aqui.

**Sem remoção de stopwords.** Em corpus técnico, palavra frequente costuma ser termo de arte. `docs/TOOLS.md` já promete isso ao chamador.

**Colisão de slug de heading é aceita.** Decidido em 2026-07-29. `Slug` funde headings distintos por projeto — é o que faz `"Art. 1.234"` funcionar — e a busca **não** desambigua: duas seções que produzem o mesmo slug são reportadas como duas ocorrências, cada uma com seu offset próprio. `matched_headings` traz o texto do heading, não o slug, justamente porque o slug não é identificador único. Não acrescente sufixo de desempate ao `Slug`: isso mudaria a resolução de âncora do M1, que já está congelada por golden files e por paridade.

**A `generation` do índice é o que invalida a busca.** `internal/index` já a incrementa em `Replace`, `Remove` e `MoveNote`. A busca não inventa um segundo contador.
...
Primeira peça do pacote `internal/search`, que não existe ainda. **`analyzer.go` é folha**: recebe texto, devolve tokens, e não importa `index`, `service` nem o SDK. Isso é o que o torna testável sem construir cofre nenhum.

**Correção de 2026-07-29:** esta seção dizia que o **pacote** `internal/search` é folha. Está errado e a revisão do M3 acusou como violação algo que a arquitetura especifica. `bm25.go` precisa de título e headings para aplicar os pesos por campo de `ARCHITECTURE.md` §6.2, e `snippet.go` precisa dos offsets — os dois consomem `internal/index` de propósito. Folhas são `analyzer.go` e `persist.go`, e devem continuar sendo. O que segue proibido é qualquer arquivo de `internal/search` importar `internal/service`.
...
## M3.1 — Correções da revisão do M3 (Tasks 51–53)

A revisão de 2026-07-29 das Tasks 43–50 encontrou um Critical de fiação, uma medição que não media o que dizia, e um lote mecânico. O gate estava verde e o `golangci-lint` zerado; nenhum dos dois pega o que está abaixo.

**Duas coisas que a revisão errou e já foram corrigidas**, para não serem re-litigadas: `internal/watcher` importar `internal/search` **não** é violação — `ARCHITECTURE.md` §5.3 põe `search.Update(nota)` no pipeline. E `internal/search` importar `internal/index` também não — §6.2 exige pesos por campo, que precisam de título e headings. O texto do §1 que dizia o contrário foi corrigido, e o "é folha" do brief da Task 44 também.

**Sequência:** 51 → 52 → 53. A 51 muda a assinatura de `watcher.New`, e a 53 mexe em arquivos que ela toca.
...
## M4 — Escrita (Tasks 54–62)

**É a única parte do produto que pode destruir dados do usuário, e vem depois de tudo por isso.** Não há backup do lado do servidor: uma nota corrompida é uma nota perdida.

**RNF-11 é o critério de bloqueio:** zero notas corrompidas em 1.000 iterações de crash injetado. Nenhuma outra tarefa do marco fecha antes da 55 passar.

**Sequência:** 54 → 55 → 56 → 57 → 58 → 59 → 60 → 61 → 62. A 55 depende da trava da 54; as 57 e 58 dependem da escrita atômica da 55; a 59 expõe as três; a 61 é independente e pode rodar em qualquer ponto.

### O que já está fechado e vincula todo o marco

- **`--read-only` remove as tools de escrita do `ListTools`**, não as rejeita depois. Um modelo que vê a tool na lista vai chamá-la, e receber erro é pior que não vê-la: ele tenta variações. RF-51 e a Task 60.
- **`dry_run` e `expected_hash` em todas as tools de escrita** (RF-37). `expected_hash` é o `xxhash` que o índice já guarda, sobre os **bytes crus** — mesma coisa que `index.Note.Hash`, calculado antes de `vault.StripBOM`.
- **EOL e BOM do arquivo original são preservados** (RF-38). `vault.DetectEOL` e `vault.StripBOM` já existem e o índice já guarda `Note.EOL` e `Note.BOM`. Escrever LF num arquivo CRLF produz diff de arquivo inteiro em cofre versionado por Git — problema clássico em Windows, e é o motivo de RF-38 existir.
- **Confinamento de caminho vale igual na escrita.** `vault.Resolve` e `Canonicalize` são as mesmas duas camadas; escrita não ganha caminho novo.
- **Nenhum tipo do SDK de MCP fora de `internal/mcpsrv`.** **stdout pertence ao JSON-RPC.**
- **O watcher vai ver as próprias escritas.** Escrever pelo `writer` gera evento de `fsnotify`, que passa pelo debounce e chega ao `Apply`. Isso é **correto** e não deve ser suprimido: o índice se atualiza pelo mesmo caminho de sempre. Mas significa que toda tarefa deste marco tem de considerar o que acontece quando a própria escrita volta como evento — em particular, o arquivo temporário não pode ser indexado, e o prefixo `.gobsidian-tmp-` já está no filtro de ruído do `vault` exatamente para isso.
...
## M5 — Refatoração do cofre (Tasks 63–68)

`note_move` e `note_delete`. **É o segundo marco que pode destruir dados** — mais insidioso que o M4, porque aqui uma escrita malfeita não corrompe **uma** nota: ela reescreve links em **dezenas** de notas que o usuário não pediu para tocar.

**Sequência:** 63 → 64 → 65 → 66 → 67 → 68. A 63 é pré-requisito das outras: sem ela, `note_move` só consegue reescrever metade dos links.

### As quatro decisões já fechadas

**1. Link Markdown É reescrito, e por isso a Task 63 existe.** `internal/parser/types.go` documenta a lacuna e já a atribui a este marco:

> *"hoje só são preenchidos para `LinkWiki` e `LinkEmbed`. Para `LinkMarkdown` e para embeds em grafia Markdown ficam no valor sentinela `offsetUnknown` (-1) [...] Fechar essa lacuna é trabalho de M5 (`note_move`), que é quem precisa dos offsets."*

Há uma contradição de contrato: **RF-35** diz *"reescrita de todos os **wikilinks**"*, e **`docs/TOOLS.md`** promete que a reescrita preserva *"a escolha entre wikilink e link Markdown"*. **Resolvida a favor do `TOOLS.md`**, porque é o contrato que o modelo do outro lado lê para decidir. Um `note_move` que reescreve wikilink e deixa `[texto](Civil/PONTO 03.md)` apontando para o caminho antigo é pior que um que recusa: quebra em silêncio, e o usuário só descobre ao clicar. **Corrija a redação de RF-35 na Task 68.**

**2. A reescrita passa pelos `Links` da nota de origem, não pelo `Backlink`.** `index.Backlink` tem `From`, `Anchor`, `Alias`, `Context` e `Kind` — **não tem `Raw` nem offsets**. O laço óbvio, `for _, bl := range idx.Backlinks(alvo)`, não consegue reescrever fielmente: falta o texto original. O caminho é `idx.Get(bl.From).Links`, onde cada `ResolvedLink` embute o `parser.Link` inteiro, com `Raw`, `Start` e `End`. Quem escrever o laço óbvio vai descobrir isso tarde.

**3. Não há transação entre arquivos, e o arquivo move POR ÚLTIMO.** `TOOLS.md` já diz que não há transação. A ordem não é indiferente: reescrever os links primeiro e falhar no move deixa dezenas de notas apontando para um caminho onde não há arquivo; mover primeiro e falhar na reescrita deixa o mesmo estrago **mais** o arquivo já deslocado. Com o move por último, qualquer falha antes dele deixa o cofre com links reescritos apontando para um arquivo que **ainda existe no lugar antigo** — inconsistente, mas nada quebrado, e uma nova execução conserta. O relatório de falha parcial tem de dizer **exatamente quais notas foram reescritas**.

**4. `.trash/` já está em `excludedDirs`** (`internal/vault/walk.go:31`). Uma nota movida para lá sai do índice pelo watcher, sozinha, sem caso especial. **Não acrescente tratamento nenhum para isso** — a propriedade já existe e código para ela seria código para um problema que não há.

### O que vale para todas as seis

- **Toda escrita usa `writer.WriteAtomic`.** Nenhuma tool deste marco abre arquivo do usuário para escrita direta. EOL e BOM preservados (RF-38), como no M4.
- **`dry_run` em todas** (RF-37), devolvendo o diff sem tocar o disco — provado por mtime, não por intenção.
- **`--read-only` remove `note_move` e `note_delete` do `ListTools`**, como o M4 fez com as de escrita. A Task 60 já tem o padrão do teste; siga-o.
- **Confinamento vale para origem E destino.** Mover para fora do cofre é a forma mais direta de exfiltrar arquivo, e `to` vem do chamador.
...
## M6 — Endurecimento (Tasks 69–77)

Último marco antes da v1.0. O escopo em altitude era H1 a H5; a ele somam-se as dívidas que as revisões dos marcos anteriores mediram e registraram, cada uma com evidência no repositório.

**Este preâmbulo NÃO chega a brief nenhum.** O extrator `task-brief` recorta de `### Task N` até o próximo `###`. Tudo que estiver aqui em cima é invisível para quem executa. Por isso cada seção abaixo repete, dentro de si, o que a vincula. Se você está lendo isto, você é quem revisa — quem executa recebeu só a própria seção.

### DECISÕES FECHADAS DO M6 — não relitigar

Estas quatro foram decididas antes de as tarefas serem escritas. Uma tarefa que as reabra está fora de escopo.

1. **O cofre sintético é gerado, não versionado.** 5.000 notas em ~50 MB não entram no repositório. `gen_vault.ps1` produz o mesmo cofre byte a byte a partir de uma semente fixa, e é a semente que é versionada. Cofre versionado incharia o clone e, pior, envelheceria em silêncio quando o gerador mudasse.
2. **Regressão de desempenho falha o build acima de 20%, e o número de referência é commitado.** Comparar contra "a última execução" torna a degradação lenta invisível: 5% por semana nunca dispara e em dez semanas dobrou. A referência é um arquivo no repositório, atualizado por commit deliberado.
3. **`bench.yml` roda em runner de CI, e o número dele NÃO substitui a medição local.** Runner compartilhado tem variância alta. O CI guarda contra regressão de ordem de grandeza; o número que vai para `docs/OPERACAO.md` continua sendo medido em máquina nomeada, com a máquina nomeada ao lado.
4. **Os quatro parâmetros de schema ignorados são IMPLEMENTADOS, não removidos.** `note_metadata.include`, `link_graph.direction`, `link_graph.include_broken` e `link_graph.include_embeds` estão documentados em `docs/TOOLS.md` com semântica clara e default declarado. Remover do schema seria encolher contrato publicado; implementar é o que o contrato já promete.

### ARMADILHAS JÁ PAGAS que valem para todo o M6

- **Não escreva número que você não mediu.** `docs/OPERACAO.md` já trouxe "~30-45 MB" como resultado de medição — era expectativa. Se não mediu, escreva **"não medido"**.
- **Um teste que não pode falhar é pior que teste ausente.** Neste projeto: o teste de paridade passava com referência vazia; `TestBuildBOM` afirmava que o heading existia e nunca o offset; e em 2026-08-01 descobriu-se que o critério de bloqueio do M4 matava o processo filho *antes* de ele escrever, relatando "0 corrompidas em 1.000 iterações" sem ter escrito um byte.
- **Prova de mutação escrita no condicional não é prova.** "Se removermos X, o teste falha" apareceu em dois relatórios e uma das duas estava factualmente errada. Rode `scripts/mutate.ps1` e cole o que ele imprimiu.
- **Medir pela camada que o requisito nomeia.** `CalculateBM25` direto deu 0,58 ms; `service.Search` deu 6–174 ms para o mesmo RNF. O requisito fala de `vault_search`, e `vault_search` é a pilha inteira.
- **Asserção de tempo não roda sob `-race`.** O detector multiplica a latência por 2 a 6. Use a constante com build tag `raceEnabled` (`internal/service/raceflag_{on,off}_test.go`) — mede nos dois modos, cobra o teto só onde ele significa algo.
